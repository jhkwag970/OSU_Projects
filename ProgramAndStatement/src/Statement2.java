...
public class Statement2 extends StatementSecondary {
    ...
    private void createNewRep() {
        StatementLabel stl = new StatementLabel(Kind.BLOCK);
        this.rep = new Tree1<StatementLabel>();
        Sequence<Tree<StatementLabel>> chl = this.rep.newSequenceOfTree();
        this.rep.assemble(stl, chl);
    }
    ...
    public final void addToBlock(int pos, Statement s) {
        ...
        Statement2 addedS = (Statement2) s;
        Sequence<Tree<StatementLabel>> chl = this.rep.newSequenceOfTree();
        StatementLabel stl = this.rep.disassemble(chl);
        chl.add(pos, addedS.rep);
        addedS.createNewRep();
        this.rep.assemble(stl, chl);
    }
    ...
    public final void assembleIfElse(Condition c, Statement s1, Statement s2) {
        ...
        Sequence<Tree<StatementLabel>> chl = this.rep.newSequenceOfTree();
        chl.add(0, localS1.rep);
        chl.add(1, localS2.rep);
        localS1.createNewRep();
        localS2.createNewRep();
        StatementLabel stl = new StatementLabel(Kind.IF_ELSE, c);
        this.rep.assemble(stl, chl);
    }
    ...
    public final Condition disassembleIfElse(Statement s1, Statement s2) {
        ...
        Sequence<Tree<StatementLabel>> chl = this.rep.newSequenceOfTree();
        StatementLabel stl = this.rep.disassemble(chl);
        localS2.rep = chl.remove(1);
        localS1.rep = chl.remove(0);
        this.createNewRep();
        return stl.condition;
    }
    ...
    public final Condition disassembleWhile(Statement s) {
        ...
        Statement2 localS = (Statement2) s;
        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();
        StatementLabel stl = this.rep.disassemble(children);
        localS.rep = children.remove(0);
        this.createNewRep();
        return stl.condition;
    }
    ...
    public final void assembleCall(String inst) {
        ...
        StatementLabel stl = new StatementLabel(Kind.CALL, inst);
        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();
        this.rep.assemble(stl, children);
    }
