    ...
    public void prettyPrint(SimpleWriter out, int offset) {
        ...
        switch (this.kind()) {
            ...
            case IF: {
                ...
                Condition ifcon = this.disassembleIf(sub);
                printSpaces(out, offset);

                out.println("IF " + toStringCondition(ifcon) + " THEN");
                sub.prettyPrint(out, offset + space);

                for (int i = 0; i < offset; i++) {
                    out.print(" ");
                }
                out.println("END IF");
                this.assembleIf(ifcon, sub);
                break;
            }
            case IF_ELSE: {
                ...
                Condition ifElsecon = this.disassembleIfElse(subIf, subElse);

                printSpaces(out, offset);
                out.println("IF " + toStringCondition(ifElsecon) + " THEN");
                subIf.prettyPrint(out, offset + space);

                printSpaces(out, offset);
                out.println("ELSE");
                subElse.prettyPrint(out, offset + space);

                printSpaces(out, offset);
                out.println("END IF");
                this.assembleIfElse(ifElsecon, subIf, subElse);
            ...
            case WHILE: {
                ...
                Statement sub = this.newInstance();
                Condition whilecon = this.disassembleWhile(sub);
                printSpaces(out, offset);
                out.println("WHILE " + toStringCondition(whilecon) + " DO");
                sub.prettyPrint(out, offset + space);

                printSpaces(out, offset);
                out.println("END WHILE");
                this.assembleWhile(whilecon, sub);
