        ...
        switch (s.kind()) {
            case BLOCK: {
                ...
                for (int index = 0; index < s.lengthOfBlock(); index++) {
                    c = s.removeFromBlock(index);
                    generateCodeForStatement(c, context, cp);
                    s.addToBlock(index, c);
                }
            ...
            case IF_ELSE: {
                ...
                Statement st1 = s.newInstance();
                Statement st2 = s.newInstance();
                Condition c = s.disassembleIfElse(st1, st2);

                cp.add(cp.length(), conditionalJump(c).byteCode());
                int Jump1 = cp.length();
                cp.add(cp.length(), dummy);
                generateCodeForStatement(st1, context, cp);
                cp.add(cp.length(), Instruction.valueOf("JUMP").byteCode());

                int Jump2 = cp.length();
                cp.add(cp.length(), dummy);
                cp.replaceEntry(Jump1, cp.length());
                generateCodeForStatement(st2, context, cp);
                cp.replaceEntry(Jump2, cp.length());
                s.assembleIfElse(c, st1, st2);
            ...
            case WHILE: {
                ...
                Instruction i = Instruction.valueOf("JUMP");
                cp.add(cp.length(), i.byteCode());
                cp.add(cp.length(), size);
            ...
            case CALL: {
                ...
                if (context.hasKey(str)) {
                    generateCodeForStatement(context.value(str),
                            context.newInstance(), cp);
                } else {
                    str = str.toUpperCase();
                    Instruction i = Instruction.valueOf(str);
                    cp.add(cp.length(), i.byteCode());
                    str = str.toLowerCase();
                }
                s.assembleCall(str);
